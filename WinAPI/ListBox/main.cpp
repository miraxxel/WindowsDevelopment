...
BOOL CALLBACK DlgProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		break;
		case IDC_BUTTON_ADD:
			// GetModuleHandle(NULL) возвр. hInstance запущенной программы
		...
		case IDOK:
		{
			...
			if (i != LB_ERR)
				...
				// выдает ошибку, и требует исп. sprintf_s, вместо этого можно ListBox -> properties -> c/c++ -> general -> SDL cheks -> NO
...
BOOL CALLBACK DlgProcAdd(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		case IDOK:
		{
			...
			HWND hParent = GetParent(hwnd); // получ. родит. окно
